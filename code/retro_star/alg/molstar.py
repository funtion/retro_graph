...
def molstar(target_mol, target_mol_id, starting_mols, expand_fn, value_fn,
    ...
    unique_smiles = set()
    for mol in mol_tree.mol_nodes:
        if not mol.open and not mol.is_known:
            unique_smiles.add(mol.mol)
    return mol_tree.succ, (best_route, i+1, len(mol_tree.mol_nodes), len(mol_tree.reaction_nodes), len(unique_smiles))
