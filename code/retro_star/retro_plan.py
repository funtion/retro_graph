...
def retro_plan():
    ...
    for (i, route) in enumerate(routes):
        ...
        result['n_mol_nodes'].append(msg[2])
        result['n_rec_nodes'].append(msg[3])
        result['n_unique'].append(msg[4])
        if succ:
    ...
    mean_mol_nodes= np.mean(result['n_mol_nodes'])
    mean_rec_nodes = np.mean(result['n_rec_nodes'])
    mean_iter = np.mean(result['iter'])

    for k in succ_thres:
        logging.info(f'Thres {k}, sucs {succ_counter[k]}, Rate {succ_counter[k]/num_targets*100:.2f}%')

    logging.info(f'Avg iter: {mean_iter}. Avg Mol nodes {mean_mol_nodes}. Avg Rec nodes {mean_rec_nodes}')
